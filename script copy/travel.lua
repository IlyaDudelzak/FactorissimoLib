...
factorissimo.on_event(defines.events.on_player_changed_surface, function(e)
    ...
    -- 1. Самые ранние и важные проверки
    if not player then return end

    -- ❗ ИСПРАВЛЕНИЕ: Используем список "god_controllers" для исключения всех режимов,
    -- которые не имеют "физической" позиции или являются наблюдателями.
    if god_controllers[player.controller_type] then
        return
    end
    
    -- Старую проверку player.spectator удаляем, она не сработала.
    -- if player.spectator then return end
    
    -- 2. Проверка поверхности и платформы
    local surface = player.surface
    if not surface or not surface.platform or not surface.platform.hub then
        return
    end

    -- 3. Основная логика перемещения
    local factory = remote_api.get_factory_by_building(surface.platform.hub)
    if not factory then return end

    -- Перемещение происходит только для активного игрока
    enter_factory(player, factory, player)
end)